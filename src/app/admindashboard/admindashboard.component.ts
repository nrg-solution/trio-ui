...
export class AdmindashboardComponent implements OnInit {
  @ViewChild('agentReport') agentReport: ElementRef; 
 ...
 // Drop down value 
  ...
  constructor(
      ...
      //  let fica: HTMLInputElement = this.report1.nativeElement;
...
onWithdrawClick(memberNumber:string,requestTypeStr:string,withdrawDate:string){
    ...
    onDeleteEmployee(employeeCode:string,employeePK:number){
        ...
        .subscribe(
            data => { 
                this.user = data;    
              //  console.log("Employee delete status ---->"+this.user.status);
                if(this.user.status=="success"){
...
clickUser(){
    ...
    DownlodagentExcel(){
        const ws: XLSX.WorkSheet=XLSX.utils.table_to_sheet(this.agentReport.nativeElement);
        const wb: XLSX.WorkBook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Sheet1'); 
        XLSX.writeFile(wb, 'AgentReport.xlsx'); 
      }
